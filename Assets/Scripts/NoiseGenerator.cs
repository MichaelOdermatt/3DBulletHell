...
{
    public static float[,] GenerateNoiseMap(int mapWidth, int mapHeight, int seed, float scale, int octaves, float persistance, float lacunarity, Vector2 offset, AnimationCurve meshHeightCurve)
    {
        ...
        for (int y = 0; y < mapHeight; y++)
        {
            for (int x = 0; x < mapWidth; x++)
            {
                ...
                noiseMap[x, y] = meshHeightCurve.Evaluate(noiseMap[x, y]);
            }
        }
